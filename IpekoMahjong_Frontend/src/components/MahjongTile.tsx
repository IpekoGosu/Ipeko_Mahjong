...
const MahjongTile: React.FC<MahjongTileProps> = ({ tile, onClick, isDrawn, className }) => {
  ...
  const isMedium = className?.includes('w-7') || className?.includes('w-8') || className?.includes('w-6');
    ...
    >
      ...
      {isZ ? (
        <span className={cn(
            ...
            isXS ? "text-[18px]" : isSmall ? "text-[21px]" : isMedium ? "text-[26px]" : "text-[42px]",
            color
        )}>
          {text}
        </span>
        ...
        <div className="flex flex-col items-center leading-none">
          <span className={cn(
              ...
              isXS ? "text-[12px]" : isSmall ? "text-[14px]" : isMedium ? "text-[18px]" : "text-[28px]",
              color
          )}>
            {text}
          </span>
